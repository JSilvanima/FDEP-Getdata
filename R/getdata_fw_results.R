...
#'
#' @title getdata_fw_results
#'
#' @description creates flowing water results dataframe from oracle data pull
#'
#' @param arg2 varible passed into SQL select statement to pull data and name data frame
#' @import RODBC
#' @import RODM
#' @import dplyr
#' @import tidyr
...
getdata_fw_results <- function(arg2) {
...
#  example above. -- getdata_fw_results("'CN2001'") --

# User will then be promoted for the password for the oracle database GWIS_ADMIN

# An SQL query is then run to pull, pivot, and and create the dataframe Results.


Results <- sqlQuery(channel, paste("select fk_station,fk_random_sample_location,collection_date,
                    sample_type,s.matrix,replace(replace(replace(replace(replace(replace(replace(replace(replace(parameter, chr(39),''),
                    chr(44),''),chr(43),''),chr(45),'_'),chr(32),'_'),chr(40),''),chr(41),''),chr(91),''),chr(93),'')
                    parameter,value,value_qualifier from t_sample s,t_parameter p,t_result r
                         where (pk_sample = fk_sample and pk_param_code = fk_param_code
                                    and substr(fk_project,3,6) = ",arg2,")"))
